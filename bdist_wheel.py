...
This file was copied from the wheel project, https://bitbucket.org/pypa/wheel,
and minor changes were made:
- inclusion of a boolean option for bdist_wheel "bytecode-only" that causes
  Python source to be stripped from the .whl.
- relative imports were changed to explicitly import from the wheel package.
"""
...
class bdist_wheel(Command):
    ...
    user_options = [('bdist-dir=', 'b',
                    ...
                    ('bytecode-only', None,
                     "remove .py files, leaving compiled bytecode in their place"
                     " (default: false)")
                    ]

    boolean_options = ['keep-temp', 'skip-build', 'relative', 'universal', 'bytecode-only']
    ...
    def run(self):
        ...
        if self.bytecode_only:
          ...
          compile_dir(build.build_base, force=True, legacy=True)
          for root, _, files in os.walk(build.build_base):
            for file in files:
              if file.endswith('.py'):
                fullpath = os.path.join(root, file)
                logger.info('Removing source file: {}'.format(fullpath))
                os.unlink(fullpath)
